...
import { validateSignIn, validateResetEmail, mapAuthError, SIGNUP_NEXT_COPY, LANDING_HEADLINE, LANDING_BULLETS, signupAcceptanceHints, profileFromSignup } from "./authForm";
import {
...
import { createDataClient, formatWriteError, formatVisitWriteError, formatProfilePhotoSaveError } from "./userData";
...
export default function Become(){
  ...
  function showVisitWriteError(action, error){
    const message=formatVisitWriteError(action, error);
    setSyncError(message);
    alert(message);
  }
  ...
  useEffect(()=>{
    ...
    dataClient.loadVisits(user.uid).then((loaded)=>{
      ...
      if(cancelled)return;
      setVisitsReady(true);
      showVisitWriteError("Couldn't load your visits", e);
    });
    return()=>{cancelled=true;};
  },[authScreen]);
  ...
  async function handleSignUp(){
    ...
    try{
      ...
      const profile=profileFromSignup(signupForm);
      await dataClient.writeProfile(result.user.uid,profile,{merge:true});
      setProfileForm(profile);
...
async function saveEditSession(){
  ...
  async function savePlannedVisit(visit){
    ...
    try{
      ...
      showVisitWriteError("Couldn't save this visit", e);
      return false;
    }finally{
      setIsSaving(false);
    }
  ...
  async function cancelPlannedVisit(visit){
    ...
    try{
      ...
      showVisitWriteError("Couldn't cancel this visit", e);
      return false;
    }finally{
      setIsSaving(false);
    }
  ...
  async function completePlannedVisit(visit){
    ...
    try{
      ...
      showVisitWriteError("Couldn't update this visit", e);
      return false;
    }finally{
      setIsSaving(false);
    }
  ...
  async function uploadProfilePhoto(file){
    ...
    setProfilePhoto(remotePhotoUrl(result.photoURL)||null);
    if(result.cancelled){
      setIsSaving(false);
      return;
    }
  ...
  const acceptanceHints=signupAcceptanceHints({attempted:signupAttempt,privacyAccepted,termsAccepted});
      ...
      {authScreen==="login"&&(
        <div className="app" style={{padding:"0 24px 48px"}}>
          ...
          <p style={{textAlign:"center",fontSize:13,color:"#9A8A78",marginBottom:32}}>
            ...
            <button onClick={()=>{setSignupMessage("");setSignupAttempt("");setAuthScreen("signup");}} style={{background:"none",border:"none",fontSize:13,fontWeight:600,color:"#B4915F",cursor:"pointer",fontFamily:"inherit"}}>Sign up</button>
          </p>
        </div>
      )}
      ...
      {authScreen==="signup"&&(
        <div className="app" style={{padding:"0 24px 48px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button className="btn btn-c" style={{flex:1,opacity:privacyAccepted&&termsAccepted?1:0.4}} disabled={!!authBusy} onClick={()=>{if(!privacyAccepted||!termsAccepted){setSignupAttempt("signup");return;}handleSignUp();}}>{authBusy==="signup"?"Creating account…":"Sign Up"}</button>
            </div>
            <div className="rule"/>
            <button className="soc" style={{opacity:privacyAccepted&&termsAccepted?1:0.4}} disabled={!!authBusy} onClick={()=>{if(!privacyAccepted||!termsAccepted){setSignupAttempt("google");return;}handleGoogleAuth(true);}}>{gLogo} {authBusy==="google"?"Connecting…":"Sign up with Google"}</button>
            {acceptanceHints.google&&<p role="status" style={{fontSize:12,color:"#C4A098",textAlign:"center",lineHeight:1.5}}>{acceptanceHints.google}</p>}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12,marginTop:4}}>
              ...
              {acceptanceHints.acceptBoth&&(
                <p role="status" style={{fontSize:11,color:"#C4A098",textAlign:"center"}}>{acceptanceHints.acceptBoth}</p>
              )}
            </div>
          </div>
        </div>
