...
export default function Become(){
  ...
  useEffect(()=>{   if(authScreen==="app"){     const user=auth.currentUser;     if(!user)return;     getDocs(collection(db,"users",user.uid,"treatments")).then(snap=>{       if(!snap.empty){         setTreatments(snap.docs.map(d=>d.data()));       }     });   } },[authScreen]);  useEffect(()=>{},[]);  const urgent=useMemo(()=>treatments.filter(t=>{
          ...
          {appTab==="profile"&&view==="home"&&(
            <div style={{paddingBottom:100}}>
              ...
              <div style={{padding:"20px",display:"flex",flexDirection:"column",gap:16}}>
                ...
                <button className="btn btn-g" onClick={async()=>{try{const {signOut}=await import("firebase/auth");await signOut(auth);}catch(e){}setAuthScreen("login");setAppTab("home");setView("home");}}>Sign Out</button>
