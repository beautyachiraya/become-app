...
export default function Become(){
  ...
  useEffect(()=>{   if(authScreen==="app"){     const user=auth.currentUser;     if(!user)return;     getDocs(collection(db,"users",user.uid,"treatments")).then(snap=>{       if(!snap.empty){         const loaded=snap.docs.map(d=>({...d.data(),sessions:d.data().sessions||[]}));         setTreatments(loaded);       }     });   } },[authScreen]);  const urgent=useMemo(()=>treatments.filter(t=>{
