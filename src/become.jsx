...
export default function Become(){
  ...
  useEffect(()=>{   if(authScreen==="app"){     const user=auth.currentUser;     if(!user)return;     import("firebase/firestore").then(({getDoc,doc:d})=>{getDoc(d(db,"users",user.uid,"profile","info")).then(snap=>{if(snap.exists())setProfileForm(snap.data());});}); getDoc(doc(db,"users",user.uid,"profile","info")).then(snap=>{if(snap.exists()){setProfileForm(snap.data());if(snap.data().photoURL)setProfilePhoto(snap.data().photoURL);}}); getDocs(collection(db,"users",user.uid,"treatments")).then(snap=>{       if(!snap.empty){         setTreatments(snap.docs.map(d=>d.data()));       }     });   } },[authScreen]);  useEffect(()=>{},[]);  const urgent=useMemo(()=>treatments.filter(t=>{
